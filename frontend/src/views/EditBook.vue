<template>
    <div style="padding: 3rem;" class="mx-auto">
        <p style="text-align: center; font-size: 2rem;" class="mb-5">Edit your book</p>
        <form class="col-11 mx-auto" @submit.prevent="handleUpdateBook">
            <div class="col-12">
                ...
                <div class="d-flex mx-2" style="flex-grow: 1">
                    <div class="col-12">
                        <textarea 
                            ...
                            class="col-12 input1" 
                            rows="15"
                            placeholder="Synopsis (min. 80 characters)"
                            v-model="synopsis"
                        ></textarea>
...
export default {
    ...
    methods: {
        getData: function () {
            axios.get(`http://localhost:9000/api/books/${this.$route.params.id}`)
                .then(res => {
                    ...
                    if (this.$store.state.user?.user?.ID != this.book.authorID) {
                         window.history.back()
                         return
                    }
                    ...
                    this.publishDate = this.book.publishDate,
                    this.genre = this.book.genre,
                    this.publisher = this.book.publisher,
                    this.editionNumber = parseInt(this.book.editionNumber),
                    this.numPages = parseInt(this.book.numPages),
                    this.maturity = this.book.maturity

                    if (this.book.coverUrl)
                        this.coverUrl = normalizeUrl("http://localhost:9000/uploads/" + this.book.coverUrl)

                    this.pdfUrl = normalizeUrl("http://localhost:9000/uploads/" + this.book.pdfUrl)
                })
                .catch(err => {
                    console.log(err)
        ...
        handleUpdateBook: function () {
            ...
            if (!this.pdf && this.isPdfChanged) {
                this.error = "Must provide a PDF File."
                window.scrollTo(0, document.body.scrollHeight)
                return
            }

            axios.put(`http://localhost:9000/api/books/${this.book.ID}`, {
                isbn: this.isbn,
                title: this.title,
            ...
            .then(async () => {
                ...
                formData.append("cover", this.cover || null)
                formData.append("pdf", this.pdf)

                if (this.isCoverChanged)
                    await fetch(`http://localhost:9000/api/books/${this.book.ID}/cover`, {body: formData, method: "put"})
        
                if (this.isPdfChanged)
                    await fetch(`http://localhost:9000/api/books/${this.book.ID}/pdf`, {body: formData, method: "put"})
